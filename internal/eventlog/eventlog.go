...
// log, and Scan to traverse the current contents of the log.
//
// After construction, a *Log is safe for concurrent access by one writer and
// any number of readers.
type Log struct {
	// These values do not change during the lifetime of the log.
	windowSize time.Duration
	maxItems   int

	// Protects access to the fields below.
	// Lock exclusive to modify the values of these fields.
	// Lock shared to read or snapshot the values.
	mu sync.RWMutex
